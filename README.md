...
- **Levels:** 1 level = 8 waves. **Wave 5 = mini-boss**, **wave 8 = main boss**. A **level-select screen** is the hub — levels unlock one at a time (clear to unlock the next), progress persists, and the game auto-resumes at your last unfinished level. Clearing the main boss → **LEVEL CLEAR** → next level unlocked. Every level starts with a **fresh setup** (no boons carried over)
- **Per-level difficulty** (config-driven, `CONFIG.levels`): two levers — **enemies** (hp/speed/spawn multipliers) and **barriers** (static crates that block the player and bullets; the Bland drift through). Level 1 is the clean focus level (no barriers); later levels are provision. Testing: `__mr.goLevel(n)` jumps to a level, `__mr.resetProgress()` wipes back to level 1
